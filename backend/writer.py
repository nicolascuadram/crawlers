...
cur = conn.cursor()
cur.execute("SELECT id, title, summary, content, url, type, published_at, created_at, log_id, sourcename, downloadArticleLink, authors FROM post")
rows = cur.fetchall()
...
for row in rows:
    (id, title, summary, content, url, type_, published_at, created_at, log_id, sourcename, downloadArticleLink, authors) = row
